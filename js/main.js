...
document.addEventListener('DOMContentLoaded', function () {
  ...
  const sliderSection = document.querySelector('.tf-slideshow');

  function checkScrollPosition() {
    if (sliderSection) {
      // Get the bottom position of the slider
      const sliderBottom = sliderSection.getBoundingClientRect().bottom;

      // If we've scrolled at least 80% past the slider
      if (sliderBottom < window.innerHeight * 0.2) {
